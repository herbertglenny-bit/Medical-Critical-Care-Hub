...
# --- 2. MOTOR IA (SISTEMA ANTIFALLO 404) ---
def get_working_model():
    try:
        ...
        # Probamos los nombres de modelo más comunes hasta que uno responda
...
model_ia, active_model_name = get_working_model()

# --- 3. BASE DE DATOS (ESTRUCTURA MÍNIMA) ---
def get_db():
...
def init_db():
    with get_db() as conn:
        # Creamos solo lo estrictamente necesario
...
# --- 4. PLANTILLA HTML V71 (ESTABILIDAD TOTAL) ---
html_template = """
<!DOCTYPE html>
<html lang="es">
<head>
...
if modo_admin:
    ...
    if file and st.button("🚀 PROCESAR"):
        if not model_ia:
            st.error("No se pudo conectar con la IA. Revisa tu clave API.")
        else:
            with st.spinner(f"Analizando evidencias con {active_model_name}..."):
                pdf_bytes = file.read()
                
                # Prompt Técnico Directo (Sin metadatos complejos)
                ...
                No uses saludos ni introducciones informales."""
                
                p_html = "Genera un fragmento DIV HTML de póster médico técnico. Blanco/Negro/Amarillo UCI."
    ...
    if g:
        ...
        render = render.replace("__MODEL__", active_model_name.replace("models/", ""))
